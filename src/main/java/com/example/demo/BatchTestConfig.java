...
public class BatchTestConfig {
    ...
    * reader는 파티셔너로부터 받은 데이터를 기반으로 읽어오는대 이때 청크 횟수 만큼 데이터를읽어온다.
    * 예를들어서 청크가 10이면 10번 reader가 데이터를 읽어오는것.-->단순 10번일수도있고 쨋든 chunk갯수만큼 데이터를 읽어오면된다.
    *
    * 아래의 jpapagingreader의 경우 알아서페이징처리까지 하는대 만약 가져온 데이터가 청크갯수와 불일치시 다시 청크갯수를 맞추기위해서
    * db하고 다시 데이터를 주고받는 문제가 존재함. 즉 chunk와 가져오는 데이터 사이즈는 웬만하면 맞추어주는게좋다.
    *
    * 그후에 processor는 10개의 데이터를 처리하고
    * writer는 이 10개에대해서 한꺼번에 처리한다
    * 이걸 reader가 데이터가 없다고 알려줄때까지 진행한다.
