...
public class BatchTestConfig {
    ...
    *
    *
    * 아래의 reader,processor,writer와 청크의 관계
    * 프로 세서는 청크만큼 reader로부터 read메서드로 데이터를 가져온다.
    * 만약 reader에 db로부터 데이터를 가져오는 과정이 존재한다면은 첫번쨰 가져올떄 chunk사이즈 만큼 가져오게 로직을 짜고
    * 따로 buffer에 저장해서 로컬메모리에서 read시 데이터를 반납하게 하는것이 db자원으ㅏㄹ 아끼고 좋은거다 이말.
    *
    *
    * 그후에 processor는 10개의 데이터를 처리하고
    * writer는 이 10개에대해서 한꺼번에 처리한다
    * 이거를 파티셔너 로부터 받은 마지막값까지 reader가 처리하고 만약 더이상 가져올게없으면 처리를 안하고 해당 substep은 종료되는 구조.
    * */

    /*
    * 메타데이터
    *
    * db에는 batch실행결과에 대한 다양한 메타 데이터가 쌓인다.
    *
    * 우선 job,step에 대한 context정보가 기록되는대 이 컨텍스트는 추후에 재실행시에도 그대로 가져와서 실행된다.
    * 단 db환경은 그때와는 다를수있다.
    *
    * job을 같음을 판별하는건 job 이름+ jobparameters로 구성된다.  만약 2개가 같다면은 같은 job 아니면 다른 job인셈이다.
    *
    * */
